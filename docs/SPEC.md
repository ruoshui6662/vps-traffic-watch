...
- 版本：1.0（t1 架构评审与规格定义）
- 作者：researcher（架构师）
- 状态：已评审，可作为后端/前端/运维的直接实现依据
- 目标平台：Linux VPS（Debian/Ubuntu/CentOS/Alpine 系）；开发环境 Windows（Python3 可直接运行调试）

---

## 1. 概述

单机部署的轻量监控服务：每 `interval` 秒采集一次所选网卡的**内核累计收发字节数**、CPU 使用率、内存与磁盘占用，写入 SQLite；通过 Flask 提供 JSON API 与一个 ECharts 仪表盘页面，浏览器访问 `http://<IP>:<port>` 查看月度/日度流量、实时速率与系统状态。
...
- **零外部依赖的部署形态**：除 Flask/psutil 外无系统级依赖；前端静态文件本地托管（ECharts 本地 vendor，避免 VPS 无外网时 UI 白屏）。
...
└── vpsmon\                         # Python 包
    ├── __init__.py                 # 空文件，声明包
    ├── app.py                      # 入口：加载配置→初始化存储→启动采集线程→Flask run
    ├── config.py                   # 配置加载/校验/默认值回退
    ├── collector.py                # 采集线程：psutil 采样→写库；网卡自动选择
    ├── storage.py                  # SQLite：建表/WAL/写样本/聚合查询（正增量算法在此）
    ├── api.py                      # Flask Blueprint：6 个 API 端点 + token 鉴权
    └── static\
        ├── index.html              # 仪表盘单页
        ├── css\style.css           # 样式
...
| `collector.py` | psutil 采样循环（独立线程）；网卡自动选择 | `select_iface(config, prefer=None) -> str`、`run_collector(storage, cfg, stop_event)` | psutil |
| `api.py` | Flask Blueprint `/api`；token 鉴权 | `bp = Blueprint('api', __name__)`；`require_token` 装饰器 | Flask |
| `app.py` | 组装与启动 | `main()`：解析 `--config/--db` → load_config → Storage.init_db → select_iface → 启动采集线程 → `app.run(host='0.0.0.0', port=port)` | 上述全部 |

**数据流**：

```
collector.run_collector（线程）
  └─ psutil 采样（网卡/CPU/内存/磁盘）
  └─ storage.insert_sample(rec)          → samples 表（唯一写入者）
Flask 线程（读）
  └─ api 端点 → storage.query_* → 正增量聚合 → JSON
```
...
- [ ] `vpsmon/config.py` — 配置加载/回退（§4）
- [ ] `vpsmon/storage.py` — DDL/WAL/正增量聚合（§5、§8.1）
- [ ] `vpsmon/collector.py` — 采样循环/网卡选择（§8.2、§8.3）
- [ ] `vpsmon/api.py` — 6 端点 + 鉴权（§6）
- [ ] `vpsmon/app.py` — 组装/启动/--config/--db（§7）
- [ ] `vpsmon/static/*` — 仪表盘（§6.1 token 交互、§8.4 单位换算）
- [ ] `install.sh` + `vpsmon.service`（§9）
- [ ] `requirements.txt`（§2）
- [ ] `README.md`（安装/API/时区口径/风险说明）
